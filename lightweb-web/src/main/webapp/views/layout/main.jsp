...
<script type="text/javascript">
	...
	function resize(){
		...
		var rightW = $(window).width() - $("#middle-left").width();
		$("#middle,#middle-left,#middle-right,#rightFrame").height(bodyH);
		$("#middle-right").width(rightW);
	}
...
<body>
				...
				<c:if test="${res.level == 1}">
					<li>
						<a href="javascript:;" id="sys_${res.id}" onclick="changeList('sys_${res.id}')">
							<img alt="${res.resourceName}" src="${rs}/images/layout/${res.resourceIcon}.png">
							<br>${res.resourceName}
						</a>
					</li>
				</c:if>
	...
	<div id="middle" style="width:100%;">
			...
			<dl class="leftmenu">
				...
				<c:if test="${res.level == 2}">
					<dd style="display:none;" class="sys_${res.parentId}_${res.id}">
						<div class="title">
							<span></span><img alt="" src="${rs}/images/layout/leftico01.png">${res.resourceName}
						</div>
						<ul class="menuson" style="display:none">
								<c:forEach var="r" items="${resList}">
									<c:if test="${r.level == 3 && r.parentId == res.id}">
										<li><cite></cite><a href="${ctx}/${r.resourcePath}" target="rightFrame">${r.resourceName}</a><i></i></li>
									</c:if>
								</c:forEach>
							</ul>
					</dd>
		...
		<div id="middle-right" style="float:left;overflow:auto;">
