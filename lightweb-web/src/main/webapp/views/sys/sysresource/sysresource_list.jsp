...
<script>
		...
		if(ids){
			top.showConfirmDiaglog('提示','删除数据不可恢复，确定要删除吗？',function(){  //关闭事件
				refleshData('SysResourceList');
				 ...
				 $.post('${path}/sysResourceController/deleteById.do?ids='+ids,function(json){
	...
	function deleteById(id){
		top.showConfirmDiaglog('提示','删除数据不可恢复，确定要删除吗？',function(){  //关闭事件
				refleshData('SysResourceList');
				 ...
				 $.post('${path}/sysResourceController/deleteById.do?ids='+id,function(json){
	...
	function changeList(pid,parentName,dictType){
		if(!selectNode){
			...
			selectNode.open ? zTree.expandNode(selectNode,false,true) : zTree.expandNode(selectNode,true);
	...
	function zTreeOnAsyncSuccess(event, treeId, treeNode, msg){
		if(!load){
			...
			load = true;
			$('#SysResourceList').bootstrapTable('refresh',{url:"${path}/sysResourceController/list.do?parentId=1"});
		}else{
			if(!treeNode){
				var nodes = zTree.getNodesByParam("pId", 0, null);
				$.each( nodes, function(i, n){
...
<body>
    ...
    <div class="rightinfo">
		...
		<div id="resource-right">
	    	...
	    	<div id="bootTb" style="overflow: auto">
	    		<table id="SysResourceList" data-toggle="table"
				data-url="" data-pagination="true"
				...
				<thead>
					<tr>
						...
						<th data-field="parentId" data-visible="false"></th>
