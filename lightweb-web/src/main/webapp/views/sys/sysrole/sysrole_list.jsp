...
<script>
		...
		if(ids){
			top.showConfirmDiaglog('提示','删除数据不可恢复，确定要删除吗？',function(){  //关闭事件
				refleshData('SysRoleList');
				 ...
				 $.post('${path}/sysRoleController/deleteById.do?ids='+ids,function(json){
				    if(json.success){
	...
	function deleteById(id){
		top.showConfirmDiaglog('提示','删除数据不可恢复，确定要删除吗？',function(){  //关闭事件
				refleshData('SysRoleList');
				 ...
				 $.post('${path}/sysRoleController/deleteById.do?ids='+id,function(json){
    ...
    function setUser(){
    	var selected=getSelectedRowsArr('SysRoleList');
    	if(selected.length>0&&selected.length<2){
    		var sel = getRow('SysRoleList');
    		var dialog = art.dialog.open("${path}/sysRoleController/toUserTree.do?roleId="+selected+"&roleType="+sel.roleType,{
    	  		  id:"selectResourceDialog",
    	  		  title:"选择用户",
    	  		  width :'300px',
    	  		  height:'380px',
...
<!-- 						</select> -->
	    ...
	    <div id="toolbar" class="btn-group">
			...
			<shiro:hasPermission name="SysRole:setUser">
				<button class="btn btn-warning btn-round" onclick="setUser();">
